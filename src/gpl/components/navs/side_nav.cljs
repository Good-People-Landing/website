(ns gpl.components.navs.side-nav
  (:require ["@heroicons/react/24/outline" :as icons]
            [gpl.hooks.use-hover-animations :refer [use-hover-animations]]
            [gpl.hooks.use-toggle :refer [use-toggle]]
            [gpl.hooks.use-toggle-animations :refer [use-toggle-animations]]
            [gpl.hooks.use-window-resize :refer [use-window-resize]]
            [gpl.lib.defnc :refer [defnc]]
            [gpl.providers.main-provider :refer [use-main-state]]
            [gpl.services.router :refer [site-map]]
            [gpl.utils.gsap :as gsap]
            [gpl.utils.window :as win-utils]
            [helix.core :refer [$]]
            [helix.dom :as d]
